...
namespace CoreWCF.OpenApi
{
    ...
    public static class OpenApiSchemaBuilder
    {
        ...
        {
            var keySchema = "";
            var typeNameSpace = $"{type?.Name}";
            if (parentDataContractAttribute?.Name != null && isInArray)
                keySchema = $"{parentDataContractAttribute.Name}-Array-{dataContractAttribute?.Name ?? typeNameSpace}";
            else if (parentDataContractAttribute?.Name != null)
                keySchema = $"{parentDataContractAttribute.Name}-{dataContractAttribute?.Name ?? type?.Name}";
            ...
            IEnumerable<(PropertyInfo, DataMemberAttribute)> properties)
        {
            foreach (var (property, dataMemberAttribute) in properties)
            {
                ...
                ProcessTypeToSchema(parentDataContractAttribute ?? dataContractAttribute, dataContractAttribute, name,
